extends Node
@onready var penis_clock: Timer = $"penis clock"
...
func cutscene1() -> void:
	get_tree().call_group("Player", "healtofull")
	incut1 = true
	GlobalVariables.cutscenemode = true
	get_tree().call_group("Player", "stop_moving")
...
func cutscene6() -> void:
	...
	if GlobalVariables.metmage == false:
		TalkScenes.mage_talk.dialogue_resource = load("res://dialogue/mage_intro.dialogue")
		TalkScenes.mage_talk.start()
	elif GlobalVariables.metmage == true:
		penis_clock.start(1)
		await penis_clock.timeout
		get_tree().call_group("enemies", "tele_in")
		get_tree().call_group("boss", "tele_sound")
		get_tree().call_group("VagabondActor", "block_l")
		...
		await penis_clock.timeout
		MusicController.vol_reset()
		MusicController.play_fight2_intro()
		cutscene6_end()
